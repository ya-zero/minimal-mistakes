...
7,7,1,0,,7,7,3
```
После обновления драйверов для igb/ixgb  перестал писать кол. потоков/очередей при загрузке модуля.
раньше были параметры InterruptThrottleRate IntMode RSS, но сейчас их нет .Но на одной из карт поддержки нет. Получается что по умолчанию у меня 8 очередей на 2 cpu / по 4  потока. пока оставлю так.
...
```
<a href="#очереди">нужно прописывать параметры через modprobe ixgb </a></p>
